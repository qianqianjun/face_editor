...
def _sanitize_tf_config(config_dict: dict = None) -> dict: # 设置相关环境参数
    # Defaults.
    cfg = dict()
...
def init_tf(config_dict: dict = None) -> None:
    ...
    for key, value in list(cfg.items()):
        # env.TF_CPP_MIN_LOG_LEVEL 只有这一个，设置日志级别的
        fields = key.split(".")
        if fields[0] == "env":
            assert len(fields) == 2
    ...
    create_session(cfg, force_as_default=True) # 创建session并且设置为默认的session
...
def convert_images_to_uint8(images, drange=[-1,1], nchw_to_nhwc=False, shrink=1, uint8_cast=True):
    ...
    """
    images = tf.cast(images, tf.float32) # 进行数据类型转换
    if shrink > 1:
